...
func replay_to_current_frame() -> void:
	...
	for frame_id in range(replay_frame, log_data.frames[replay_peer_id].size()):
		...
		if frame_data.type == DeltaRollbackLogger.FrameType.TICK and frame_data.data.get('skipped', false):
...
func _send_replay_frame_data(frame_data: LogData.FrameData) -> void:
	...
	if frame_type == DeltaRollbackLogger.FrameType.TICK:
	...
	elif frame_type == DeltaRollbackLogger.FrameType.INTERPOLATION_FRAME:
