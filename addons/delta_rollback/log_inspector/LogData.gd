...
func _loader_thread_function(input: Array) -> void:
	...
	while not file.eof_reached():
		...
		if header == null:
			if data['log_type'] == DeltaRollbackLogger.LogType.HEADER:
				header = data
...
func _add_log_entry(log_entry: Dictionary, peer_id: int) -> void:
	...
	match log_entry['log_type'] as int:
		DeltaRollbackLogger.LogType.INPUT:
		...
		DeltaRollbackLogger.LogType.STATE:
		...
		DeltaRollbackLogger.LogType.EVENT:
		...
		DeltaRollbackLogger.LogType.FRAME:
